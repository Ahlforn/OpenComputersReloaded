...
/**
 * A node that is both a {@link li.cil.oc.api.network.Component} (callable from computers) and a
 * {@link li.cil.oc.api.network.Connector} (energy buffer). Extends {@link ConnectorNodeImpl} for the
 * energy behaviour and delegates component callback discovery/dispatch to {@link ComponentSupport}.
 */
class ComponentConnectorNodeImpl extends ConnectorNodeImpl implements ComponentConnector {

    private final String name;
    private Visibility visibility;
    private final ComponentSupport support;

    ComponentConnectorNodeImpl(Environment host, Visibility reachability,
                               String name, Visibility visibility, double bufferSize) {
        super(host, reachability, bufferSize);
        this.name = name;
